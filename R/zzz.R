...
add_glims_mo <- function(...) {
  tryCatch({
    ...
    if (length(add_mo_manually) > 0) {
      packageStartupMessage("Adding ", length(add_mo_manually), " microorganisms from `certetoolbox:::mo_table_glims` to the AMR package...", appendLF = FALSE)
      suppressMessages(
